...
> 更新时间：2026-08-12（扩展融合 68.95% 当前最优 + 方向 4 证伪落档）
...
- **线上准确率当前最优 = 68.95%**（**扩展融合包** `output/fuse_c2_r4_c/pred_results.zip` = c2(67.86%) × round4(67.86%线检查点) × c(64.66%) 三方等权概率融合，2026-08-12 官网确认，比原融合基线 68.37% +0.58pp、比 d×c2×c 68.43% +0.52pp）。
- 历史分数线：扩展融合 **68.95%** → d×c2×c **68.43%** → 融合 c2×c **68.37%** → c2 阶段 **67.86%** → 阶段 C **64.66%** → 阶段 B **63.66%** → 旧 V2 **55.02%** → V1 **48%** → 阶段 A **34%**（已废弃，见 §2）。
...
3. **候补融合变体**（低成本，可试）：`c2 × round3 × c`、`c2 × round5 × c`、或 α 加权调优（强检查点给更大权重）；`d × c2 × c`（68.43%）已证 d 互补但弱于 r4 版。
4. 以上均需以官网提交分为唯一验收标准。
...
| 包 | 组合 | 线上 |
|----|------|------|
| `fuse_c2_r4_c` | c2 × round4 × c | **68.95%**（最优）|
